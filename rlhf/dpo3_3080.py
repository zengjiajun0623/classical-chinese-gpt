...
picks3=["a","a","b","b","a","a","b","b","a","b","b","a","a","b","a","b","b","a","a","b"]
sets=[("pairs.json",picks1),("pairs2.json",picks2),("pairs3.json",picks3)]
data=[]
for fn,pk in sets:
    prs=json.load(open(D+"\\"+fn,encoding="utf-8"))
    for pr,p in zip(prs,pk):
        chosen=pr[p]; rejected=pr["b" if p=="a" else "a"]
...
for ep in range(EPOCHS):
    ...
    for j in order:
        ...
        kl+=abs((pc-rc).item())/max(1,len(ci))
    print("epoch %d  dpo_loss %.4f  chosen>rejected %d/%d  ref_drift/tok %.4f" %
          (ep+1, tot/len(data), acc, len(data), kl/len(data)), flush=True)

torch.save({"model":policy.state_dict()}, D+r"\gpt2_zh_dpo3.pt")
print("saved gpt2_zh_dpo3.pt", flush=True)
