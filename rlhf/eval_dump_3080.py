...
def run_qa(ck,use_rag):
    ...
    for q,gold in QS:
        ...
        ok=gold in a
        RESULTS.append({"model":ck,"track":"facts","q":q,"gold":gold,"answer":a,"pass":ok})
        if ok: facts+=1
    ...
    for e in HONESTY:
        ...
        a=gen(m,qa_prompt(q,use_rag))
        ok="不知道" in a
        RESULTS.append({"model":ck,"track":"honesty","q":q,"gold":"不知道","answer":a,"pass":ok})
        if ok: hon+=1
...
def run_style(ck):
    ...
    for p in STYLE:
        a=gen(m,p,n=90)
        RESULTS.append({"model":ck,"track":"style","q":p,"gold":"stay in "+ANCHOR[p],"answer":a,"pass":len(worlds(a)-{ANCHOR[p]})==0})
...
import io
json.dump(RESULTS, open(D+r"\results_day1.json","w",encoding="utf-8"), ensure_ascii=False, indent=1)
print("dumped %d rows to results_day1.json" % len(RESULTS))
